...
void draw_player_level() {
    ...
    level_text += " out of ";
    level_text += std::to_string(LEVELS.size());
...
void derive_graphics_metrics_from_loaded_level() {
    ...
    cell_size = std::min(
        screen_width  / static_cast<float>(level.getColumns()),
        screen_height / static_cast<float>(level.getRows())
    ) * CELL_SCALE;
    screen_scale = std::min(
        screen_width,
        screen_height
    ) / SCREEN_SCALE_DIVISOR;
    float level_width  = static_cast<float>(level.getColumns()) * cell_size;
    float level_height = static_cast<float>(level.getRows())    * cell_size;
...
void draw_player() {
    float x = shift_to_center_cell_by_x + static_cast<float>(player.getColumn()) * cell_size;
    float y = shift_to_center_cell_by_y + static_cast<float>(player.getRow())    * cell_size;
